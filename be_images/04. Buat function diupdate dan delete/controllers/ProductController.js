...
export const updateProduct = async (req, res) => {
  ...
  // Kondisi jika tidak id atau data
  ...
  if (req.files == null) {
    ...
    if (!allowedType.includes(ext.toLocaleLowerCase()))
      return res.status(422).json({ message: "Images Invalid" });
    if (fileSize > 5000000)
      return res
        .status(422)
        .json({ message: "Images must be less  than 5 MB" });

    const filepath = `./public/images/${product.image}`;
    fs.unlinkSync(filepath);

    file.mv(`public/images/${fileName}`, (err) => {
      if (err) return res(500).json({ message: err.message });
    });
  }
